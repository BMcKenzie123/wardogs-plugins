...
/** Aggregates the stats-logger files into data/leaderboard.json (all-time kills, deaths, minutes, sessions). */
export default definePlugin<Options>({
  ...
  setup(ctx) {
    ...
    ctx.every(
      Number(ctx.options.everyMinutes) * 60_000,
      async () => {
    ...
    ctx.log.info(`rebuilding top ${ctx.options.top} every ${ctx.options.everyMinutes} min`);
  },
});
