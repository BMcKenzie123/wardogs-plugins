...
| `admin-panel`             | The all-in-one page: the dashboard plus DM / kick / kill / ban / move per player, broadcast, map change, lighting, match end/restart, bans, reserved slots, sponsor banner, audit log, and plugin management (enable/disable, edit every option live, restart, reset). HTTP Basic auth | `path`, `auditRows`, `logLines`                                        | `HTTP_PORT`, `ADMIN_PASSWORD` |
