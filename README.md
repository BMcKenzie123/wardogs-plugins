...
[docs/DISCORD.md](docs/DISCORD.md) (the WARDOGS Discord: the case made to TAW, the build sheet, where each proposal item stands).
...
| `audit-tail`      | Logs each new admin-action audit entry; optionally to `data/audit.jsonl`                              | `toFile`, `skipNoise`                                                                      | —                        |
...
| `kill-streak`             | Announces kill milestones within a match, to everyone or as a whisper to the player                                                                                                                                                                                                                                                                                                                                                                                        | `mode`, `thresholds[]`, `message`                                      | —                             |
| `first-timer`             | Welcome for a never-seen steamId: a broadcast, or a whisper to the newcomer                                                                                                                                                                                                                                                                                                                                                                                                | `mode`, `delayMs`, `message`                                           | —                             |
| `playtime-ranks`          | Rank-ups by total hours played: broadcast, or whispered on the player's next join                                                                                                                                                                                                                                                                                                                                                                                          | `mode`, `ranks[{hours,title}]`, `announce`                             | —                             |
...
| `admin-panel`             | The all-in-one page: the dashboard plus DM / kick / kill / ban / move per player, broadcast, map change, lighting, match end/restart, bans, reserved slots, sponsor banner, audit log, Steam names and avatars for every SteamID (with `STEAM_API_KEY`), temp bans with a length and a Cleanup button that lifts expired ones, and plugin management (enable/disable, edit every option live, restart, reset). Sections update in place without reloading. HTTP Basic auth | `path`, `auditRows`, `logLines`                                        | `HTTP_PORT`, `ADMIN_PASSWORD` |
...
Plugin management lives in the panel's **Automation** section. Every plugin has an Enable/Disable toggle and a
**Configure** drawer listing each option with its current value and its default: numbers, booleans, text,
one-per-line lists, and JSON for structured options such as tiers. **Save & apply** writes only the values that
differ from the defaults to `plugins.json` and restarts that one plugin in place (no service restart, other
plugins untouched); **Reset to defaults** drops the entry, **Restart** bounces a running plugin. Bad input
(a non-number, invalid JSON) is rejected with the field named and nothing is written. The panel refuses to
reconfigure or disable itself; edit its own entry in `plugins.json` and restart the service for that.
The page updates itself in place every 8 seconds (players, plugin states, activity, bans, audit, stats): no reload,
no scroll jump, and any section you are typing in or have a drawer open in is left untouched until you are done.
