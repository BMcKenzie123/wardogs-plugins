...
| **Recruiting lead**     | _assign_                | The WARDOGS Discord: the approval through the chain of command with the case in [DISCORD.md](DISCORD.md), the server built to the build sheet, webhooks to the server lead privately. The recruiting copy after v2.0. Funnel numbers.                                     | Read [DISCORD.md](DISCORD.md); take the proposal to TAW leadership; create the server when approved.                                 |
...
| **Admins (moderation)** | `jdam`, `ralf`, `bqmck` | Day-to-day: kicks, bans, DMs, force MOTD, toggling plugins. Everything they do is logged under their own login and recorded in `data/admin-actions.jsonl`. Answer ban appeals and player reports in the appeals channel within 24 h; a ban without a reason is not a ban. | Sign in at the panel with your own name; if a password is missing, ask the server lead (rotate with `wd admin-hash <name>`).         |
| **EU server lead**      | _assign_                | The EU game server: its RCON details on the box (blocker #13), EU rules/event slot, EU seeding crew and admins (#14).                                                                                                                                                     | Get the EU RCON host, port and password from the EU host's panel to the server lead; sign in at `/eu/admin`.                         |
| **EU seeding crew**     | _assign (2+ people)_    | The EU server is never empty at its agreed slot.                                                                                                                                                                                                                          | Agree an EU slot; it goes into the EU instance's event-announcer.                                                                    |
