...
| TAW rank-based permissions, consistent permissions, onboarding/offboarding | Planned: the bot maps Discord roles to command permissions (viewer / moderator / admin / owner); a role change on Discord is the permission change. The panel keeps its own logins for the box.                                  | bot (#3), dev plan v2.2            |
...
| Member verification (Discord to Steam)                                     | Planned: `/link <steamId>` creates a pending link; an admin confirms, or the player proves the Steam account by putting a one-time token in their Steam profile name. An in-game code is not possible: the API cannot read chat. | bot (#3), dev plan v2.2            |
