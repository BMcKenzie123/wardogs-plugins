...
- **`{discord}` is the only link.** It expands to `DISCORD_INVITE` on the box (today discord.gg/taw; the WARDOGS
  Discord once it exists). One destination, always the same, changed in one place.
...
| #   | Line                                                                                               | Job                  |
| --- | -------------------------------------------------------------------------------------------------- | -------------------- |
| 1   | TAW WARDOGS NA: the new-player-friendly WARDOGS server. Squad up with us on Discord: {discord}     | who we are, the link |
| 2   | Rules here are simple: play fair, no cheating, no harassment, no griefing. Admins are in {discord} | rules (pending #1)   |
| 3   | Good games need players. Bring one friend tonight and this server fills twice as fast.             | the invite ask       |
| 4   | Want a reserved slot and a squad that shows up every night? That is what TAW is. {discord}         | the recruiting ask   |
...
- **message** `{server} {name} {players} {max}`:
  `Welcome to TAW WARDOGS NA, {name}. {players}/{max} on right now. New here? Say hi in chat. Discord: {discord}`
- **returningMessage** `{name}`:
  `Welcome back, {name}. Good to see you again. Discord: {discord}`
...
`Thank you for seeding with us today, {name}! Remember to invite your friends for better quality games faster, and join us on Discord at {discord}`

## Recruiting pitch (`recruit-pitch`, DM after 15 min of real play, once a week per player)

- **message** `{name} {server} {players} {max} {kills}`:
  `Enjoying TAW WARDOGS NA, {name}? TAW is a real unit: squads, ranks, people who show up every night. See for yourself: {discord}`

## Regulars (`regulars`, DM on the visit that hits a tier)

- **visit 3** `{name} {visits} {minutes} {server}`:
  `Third visit, {name}. That makes you a regular around here. Come meet the rest of us: {discord}`
- **visit 10**:
  `{visits} visits and {minutes} minutes with TAW WARDOGS NA, {name}. Regulars like you get reserved slots. Ask in {discord}`

## Comeback (`comeback`, DM when someone returns after 14+ days)

- **message** `{name} {days}`:
  `Welcome back, {name}. {days} days is too long. Catch up with the crew: {discord}`

## Match MVP (`match-mvp`, off until approved)

- **broadcast** `{map} {list}`: `Match MVPs on {map}: {list}. GG all.`
- **mvpMessage** `{name} {kills} {deaths}`:
  `MVP, {name}. {kills}K/{deaths}D. TAW recruits players like you. Come see what we are about: {discord}`
...
One-line version for the MOTD: `Rules here are simple: play fair, no cheating, no harassment, no griefing. Admins are in {discord}`
...
- No second link. Everything routes through {discord}.
...
(admin-panel option `appealNote`, default `Appeal at {discord}`), so an admin only writes the cause:

- `Cheating` → the player sees `Cheating. Appeal at {discord}`
- `Team-killing after two warnings` → `Team-killing after two warnings. Appeal at {discord}`
- a temp ban with no reason typed → `Appeal at {discord}`
...
`Rules: play fair, no cheating, no harassment, no griefing. Report a player or appeal a ban at {discord}`
