...
| **Permissions from TAW rank, via the main TAW API.** The authoritative source of who holds which rank in which unit is TAW's member system, not roles the platoon assigns itself. The bot reads a member's callsign, rank, unit and status from the main TAW API (read-only key on the box, `TAW_API_KEY`, entered by silent prompt; cached 15 min) and maps rank to `viewer` / `moderator` / `admin` / `owner` through a `ranks` option. Moderator: `/kick`, `/tempban`, `/dm`, `/broadcast`, `/motd`. Admin: `/ban`, `/unban`, `/map`, `/lighting`. Owner: plugin config. | A rank change in TAW is the permission change; nobody holds a credential. Every command is written to `data/admin-actions.jsonl` under the TAW callsign and Discord user, the same file the panel uses. Fallback if API access is not granted: mirror the roles of the main TAW Discord (the bot joins it read-only) or roles assigned on the WARDOGS server by the CoC. **Blocker #17.** |
| **Member verification, via the main TAW API.** Where TAW's member record already holds a Discord id and a Steam id, the bot links them with no ceremony: a TAW member is recognised on the game server the moment they join. Where it does not, `/link <steamId>` creates a pending link that an admin confirms with `/verify`, or the player proves ownership by putting a one-time token in their Steam profile name for a minute (the Steam lookup already exists). Linked pairs live in `data/links.json`.                                                              | The game's API cannot read chat, so an in-game code is not an option. The TAW API is the difference between "verified by TAW" and "verified by us". **Blocker #17.**                                                                                                                                                                                                                      |
...
**Dependency: the main TAW API.** Rank-based permissions and member verification are only as good as their source. Reading rank, unit, status and linked ids from TAW's member system (read-only) is a request to the chain of command and TAW IT, tracked as blocker #17, and is the reason the bot is the second half of this version rather than the first.
